...
import { CreateKnowledgeSchema, UpdateKnowledgeSchema } from '../lib/schemas.js';
...
async function create(body: string | undefined) {
  ...
  const validationResult = CreateKnowledgeSchema.safeParse(parsed);
...
async function update(id: string, body: string | undefined) {
  ...
  // Validate using Zod schema
  const validationResult = UpdateKnowledgeSchema.safeParse({ ...parsed, id: numericId });
